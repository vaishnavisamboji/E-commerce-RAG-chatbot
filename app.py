...
def load_pipeline():
    # Read keys from .env file directly
    with open('/kaggle/working/.env') as f:
        for line in f:
    ...
    # ------------------------------------------------------------------
    # Download large files from S3 if not present locally
    # ------------------------------------------------------------------
    ...
    def retrieve(query, k=10):
        ...
        query_emb = query_emb / np.maximum(np.linalg.norm(query_emb), 1e-9)
        scores    = embs_matrix @ query_emb
